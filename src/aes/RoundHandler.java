 ...
 *		Some methods are overloaded to simplify calls when using encryption methods
 *			Call them with no second arg to use the encryption variant, call with decrypt=true for decryption variant
 *
 *	References:
...
public class RoundHandler {
	SBoxCalculator sb;
	int[][][] allRoundKeys; //storage for all 11 round keys that we will be using
	int rcon; //starts at 1, doubles in GF(2^8) for each key --- should go (in hex) 01 02 04 08 10 20 40 80 1b 36
	int[][] mixMatrix = { //matrices used for multiplication in mixColumns step
		{2,3,1,1},
		{1,2,3,1},
		{1,1,2,3},
		{3,1,1,2}
	...
	public RoundHandler(int[][] key) {
		...
		generateRoundKeys(key); //Generates all 11 keys and saves it to allRoundKeys
	}
	...
	public int[][] shiftRows(int[][] input, boolean decrypt) {
		/*
		 * shifts row 2 left by 1, row 3 left by 2, row 4 left by 3 if encrypting
		 * shifts right similarly if decrypting
		 * call with decrypt = true if decrypting, decrypt = false for encrypting, or just call the overloaded method 
		 */
		int[][] output = new int[4][4];
		for (int i = 0; i < 4; i++) {
	...
	/*
	 * Decryption and Encryption is the same for round key, we just use the keys in inverse order to decrypt
	 */
	...
	public int[][] addRoundKey(int[][] input, int round) {
		/*
		 *  Adds roundkey for given round to the input matrix
		 */
